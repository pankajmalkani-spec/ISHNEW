...
export default function SubcategoryEdit({ authUser = {}, subcategoryId }) {
    ...
    const verifySort = async (value) => {
        setSortError('');
        if (!value) return;
        const { data } = await axios.get('/api/mwadmin/subcategories/verify-sort', { params: { sort: value, ignore_id: subcategoryId } });
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            await axios.post(`/api/mwadmin/subcategories/${subcategoryId}?_method=PUT`, payload, {
            ...
            setError(err?.response?.data?.message || 'Unable to update sub-category.');
        } finally {
            setSaving(false);
        }
    };
                    ...
                    <section className="mwadmin-panel mwadmin-form-panel">
                        ...
                        {loading ? <div>Loading...</div> : (
                            <form onSubmit={onSubmit} className="mwadmin-form-grid">
                                ...
                                <div className="mwadmin-upload-card"><label>Banner Image</label><input type="file" accept="image/*" onChange={(e) => setForm((f) => ({ ...f, banner_img: e.target.files?.[0] || null }))} /></div>
                                <div className="mwadmin-upload-card"><label>Box Image</label><input type="file" accept="image/*" onChange={(e) => setForm((f) => ({ ...f, box_img: e.target.files?.[0] || null }))} /></div>
                                <div className="mwadmin-form-actions"><button type="submit" disabled={saving}>{saving ? 'Saving...' : 'Update'}</button><Link href="/mwadmin/subcategory">Cancel</Link></div>
                            </form>
                        )}
                    </section>
                </div>
