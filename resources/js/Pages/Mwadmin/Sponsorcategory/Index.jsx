...
export default function SponsorCategoryIndex({ authUser = {} }) {
    ...
    const perms = useMemo(
        () => ({
            view: canViewDetail(authUser, 'sponsorcategory'),
            edit: canEdit(authUser, 'sponsorcategory'),
            deactivate: canDelete(authUser, 'sponsorcategory'),
            add: canAdd(authUser, 'sponsorcategory'),
        }),
        [authUser]
    );
        ...
        async (id) => {
            if (
                !(await dialog.confirm(
                    'Mark this sponsor category as inactive? It will not be removed from the database.',
                    'Deactivate Sponsor Category'
                ))
            ) {
                return;
            }
            try {
                ...
                dialog.toast('Sponsor category has been marked as inactive.', 'success');
                await loadData();
            } catch (err) {
                const msg = err?.response?.data?.message || 'Unable to deactivate sponsor category.';
                    ...
                    <section className="mwadmin-panel mwadmin-sponsorcategory-grid-panel">
                        <div className="mwadmin-toolbar">
                            ...
                            <div className="mwadmin-right-tools">
                                {perms.add ? (
                                    <Link className="mwadmin-add-btn" href="/mwadmin/sponsorcategory/create">
                                        + Add New Sponsor Category
                                    </Link>
                                ) : null}
